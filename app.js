...
function applyAudienceUI() {
    document.body.classList.toggle("has-audience", !!currentAudience);
    ...
    document.querySelectorAll(".audience-pill").forEach(pill => {
        const val = pill.dataset.setAudience === "all" ? null : pill.dataset.setAudience;
        pill.classList.toggle("active", val === currentAudience);
    });
...
function updateCopy() {
    ...
    if (currentAudience === "civilian") {
        ...
        timelineLead.textContent = "Ваш графік навчання.";
        timelineFinalDesc.textContent = "Складання випускного екзамену. Присягу ви вже склали раніше.";
    } else {
...
const roadmapCivilian = [
    { title: "Зберіть документи у фото", text: "Паспорт, довідка ІПН, довідка про реєстрацію місця проживання, військово-обліковий документ, диплом, власноруч написана заява." },
    { title: "Зберіть усе в один .docx", text: "Назва файлу: ПІБ_дата_народження.docx (наприклад, Коваленко_Степан_Петрович_12051985.docx)." },
    ...
    { title: "Зарахування та оплата", text: "Оплатіть 44 800 грн протягом двох тижнів після наказу про зарахування." }
];
...
const faqData = [
    ...
    },
    {
        ...
        <p>Дистанційна частина відбувається у зручному для вас режимі: конкретний час не встановлюється, є лише дедлайни звітності.</p>
        <p>На навчальний збір вас відряджає командир вашої частини своїм наказом. Відповідно до Закону України «Про військовий обов'язок і військову службу» (ч. 3 ст. 29) та КЗпП (ч. 2 ст. 119), на час збору за вами зберігається посада і середня заробітна плата.</p>`
    },
    {
    ...
    },
    {
        ...
        answer: `<p><strong>Ні.</strong> Навчання здійснюється виключно на платній основі за контрактом. Вартість - <strong>44 800 грн за всю програму підготовки</strong>.</p>
    ...
    },
    {
        ...
        <p>Точні етапи та формат кожного з них - у розділі «Графік навчального процесу» вище на цій сторінці.</p>`
    },
    {
        id: "faq-6",
        category: "documents",
